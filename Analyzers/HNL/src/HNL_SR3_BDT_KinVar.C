...
void HNL_SR3_BDT_KinVar::executeEvent(){
  ...
  AnalyzerParameter param_bdt = HNL_LeptonCore::InitialiseHNLParameter("BDT", "_UL");
  //AnalyzerParameter param_bdt = HNL_LeptonCore::InitialiseHNLParameter("HNL", "_UL");
  ...
  vector<HNL_LeptonCore::Channel> channels = {MuMu, EE, EMu};
  //vector<HNL_LeptonCore::Channel> channels = {MuMu};

  if(!IsData) gens = GetGens();

  std::vector<Muon>       MuonCollT     = GetLepCollByRunType    ( GetMuons    (param_bdt.Muon_Tight_ID, 10., 2.4)    , gens, param_bdt, "");
  std::vector<Electron>   ElectronCollT = GetLepCollByRunType    ( GetElectrons(param_bdt.Electron_Tight_ID, 10., 2.5), gens, param_bdt, "");
  ...
  for(auto dilep_channel : channels){
    ...
    if(!ConversionSplitting(LepsT, true)) continue;
    ...
    //std::vector<Jet>    AK4_JetAllColl = GetJets("NoID", 20., 2.7);
    //std::vector<Jet>    AK4_JetAllColl = GetJets("tight", 20., 2.7);
    ...
    //std::vector<Jet>    AK4_JetAllColl = SelectAK4Jets(jets_tmp,      20., 2.7, true, 0.4, 0.8, "",  ElectronCollV, MuonCollV, AK8_JetColl);
    ...
    if(RunMCFake) passCharge = true;
    else{
      if(SameCharge(LepsT)) passCharge = true;
    }

    //==== Merging multiple datasets for signal and DY Z
    double weightDYTypeI = 1., weightDYZ = 1.;

    if(RunMCFake || MCSample.Contains("DYTypeI")) weightDYTypeI = 0.5;
    weight *= weightDYTypeI;
