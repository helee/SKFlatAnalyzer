...
bool HNAnalyzerCore::SameCharge(vector<Muon> muons){

  if(muons.size() != 2) return false;
  if(muons[0].Charge() == muons[1].Charge()) return true;

  return false;

}

bool HNAnalyzerCore::SameCharge(vector<Electron> electrons){

  if(electrons.size() != 2) return false;
  if(electrons[0].Charge() == electrons[1].Charge()) return true;
