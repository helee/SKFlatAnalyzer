...
void HNtypeI_SR::initializeAnalyzer(){
  ...
  //==== I defined "vector<TString> MuonIDs;" in Analyzers/include/HNtypeI_SR.h
  MuonIDs = {
    "POGMedium",
    "POGTight"
  };
  ...
  cout << "[HNtypeI_SR::initializeAnalyzer] RunNewPDF = " << RunNewPDF << endl;
  if(RunNewPDF && !IsDATA){
  ...
  cout << "[HNtypeI_SR::initializeAnalyzer] RunXSecSyst = " << RunXSecSyst << endl;
...
void HNtypeI_SR::executeEvent(){
  ...
  if(RunNewPDF && !IsDATA){
    //cout << "[HNtypeI_SR::executeEvent] PDF reweight = " << GetPDFReweight() << endl;
    FillHist("NewPDF_PDFReweight", GetPDFReweight(), 1., 2000, 0.90, 1.10);
    //cout << "[HNtypeI_SR::executeEvent] PDF reweight for error set (NErrorSet = "<<pdfReweight->NErrorSet<< ") :" << endl;
    for(int i=0; i<pdfReweight->NErrorSet; i++){
      //cout << "[HNtypeI_SR::executeEvent]   " << GetPDFReweight(i) << endl;
