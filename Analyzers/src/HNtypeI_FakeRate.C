...
void HNtypeI_FakeRate::initializeAnalyzer(){
  ...
  //==== I defined "vector<TString> MuonIDs;" in Analyzers/include/HNtypeI_FakeRate.h
  MuonIDs = {
    "POGMedium",
    "POGTight"
  };
  ...
  cout << "[HNtypeI_FakeRate::initializeAnalyzer] RunNewPDF = " << RunNewPDF << endl;
  if(RunNewPDF && !IsDATA){
  ...
  cout << "[HNtypeI_FakeRate::initializeAnalyzer] RunXSecSyst = " << RunXSecSyst << endl;
...
void HNtypeI_FakeRate::executeEvent(){
  ...
  if(RunNewPDF && !IsDATA){
    //cout << "[HNtypeI_FakeRate::executeEvent] PDF reweight = " << GetPDFReweight() << endl;
    FillHist("NewPDF_PDFReweight", GetPDFReweight(), 1., 2000, 0.90, 1.10);
    //cout << "[HNtypeI_FakeRate::executeEvent] PDF reweight for error set (NErrorSet = "<<pdfReweight->NErrorSet<< ") :" << endl;
    for(int i=0; i<pdfReweight->NErrorSet; i++){
      //cout << "[HNtypeI_FakeRate::executeEvent]   " << GetPDFReweight(i) << endl;
