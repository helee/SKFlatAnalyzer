...
public:
  ...
  std::vector<Jet> JetsAwayFromFatJet(const std::vector<Jet>& jets, const std::vector<FatJet>& fatjets, double mindr=0.8);                                      // 0.8 in EXO-17-028
  ...
  std::vector<FatJet> FatJetsVetoLeptonInside(const std::vector<FatJet>& jets, const std::vector<Electron>& els, const std::vector<Muon>& mus, double dR=1.0);  // 1.0 in EXO-17-028
  ...
  bool SameCharge(vector<Muon> muons);
  bool SameCharge(vector<Electron> electrons);
  ...
  std::vector<Electron> ShiftElectronEnergy(TString ID, const std::vector<Electron>& electrons, bool applyShift);
